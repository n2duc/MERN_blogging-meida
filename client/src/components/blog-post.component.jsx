...
const BlogPostCard = ({ content, author }) => {
    const { publishedAt, tags, des, banner, title, activity: { total_likes }, blog_id: id } = content;
        ...
        <Link to={`/blog/${id}`} className="flex gap-8 items-center border-b border-grey pb-5 mb-4">
            <div className="w-full">
                ...
                <div className="flex gap-4 mt-7">
                    <span className="btn-light py-1 px-3 text-base">{tags[0]}</span>
                    <span className="ml-3 flex items-center gap-2 text-dark-grey">
                        <i className="fi fi-rr-heart text-xl mt-1" />
                        {total_likes}
                    </span>
                </div>
            </div>
            ...
            <div className="h-28 aspect-square bg-grey">
                <img src={banner} alt="banner" className="w-full h-full rounded-md aspect-square object-cover" />
            </div>
        </Link>
    )
}
