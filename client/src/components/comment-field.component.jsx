...
const CommentField = ({ action = "comment", index = undefined, replyingTo = undefined, setReplying }) => {
    ...
    const { userAuth: { access_token, username, profile_img, fullname } } = UserAuth();
    ...
    const handleComment = async () => {
        ...
        if (!comment.length) {
            return toast.error("Write somthing to comment");
        }
    ...
    return (
        <>
            ...
            ></textarea>
            <button className="btn-dark mt-5 px-10 capitalize" onClick={handleComment}>
                {action}
            </button>
        </>
    );
