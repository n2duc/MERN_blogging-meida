...
import images from "../imgs/draft-image.png"

const TabDraft = ({ username }) => {
    const { userAuth: { access_token } } = UserAuth();
    const [draft, setDraft] = useState([]);
    const [userId, setUserId] = useState("");
    const getBlogs = async (user_id) => {
        try {
            const { data } = await axios.post(`${import.meta.env.VITE_SERVER_DOMAIN}/get-draft-posts`, {
                author: user_id,
            })
            setDraft(data.blogs);
        } catch (error) {
    ...
    return (
        <div className="flex flex-col w-full gap-4">
            {draft.length ? draft.map((blog, index) => (
                <div className="flex justify-between items-start py-4 border-b border-grey" key={index}>
                    <div className="flex items-start gap-3">
                        <img src={blog.banner ? blog.banner : images} alt="banner" className="w-[120px] h-[100px] rounded object-cover mr-8 border border-grey" />
                        <div className="pt-1">
                            <p className="block leading-5 text-2xl mb-4 text-black">{blog.title}</p>
