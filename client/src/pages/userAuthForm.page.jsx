...
const UseAuthForm = ({ type }) => {
    ...
    let { userAuth: { access_token }, setUserAuth } = UserAuth();

    const userAuthThroughServer = (serverRoute, formData) => {
        axios.post(`${import.meta.env.VITE_SERVER_DOMAIN}${serverRoute}`, formData)
            .then(({ data }) => {
                storeInSesstion("user", JSON.stringify(data))
                setUserAuth(data);
            })
            .catch(({ response }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!passwordRegex.test(password)) return toast.error("Password should be 6 to 20 characters long with a numeric, 1 lowercase and 1 uppercase letters")
